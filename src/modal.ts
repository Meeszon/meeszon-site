...
function buildModalContent(data: ProjectData): string {
  ...
  const slides = gallery
    .map(
      (src, i) =>
        `<div class="modal-carousel-slide"><img ${i === 0 ? `src="${src}"` : `data-src="${src}"`} alt="Screenshot ${i + 1}" draggable="false" /></div>`,
    )
    .join("");
...
function initModalCarousel() {
  ...
  function loadSlide(index: number) {
    const slide = slides[((index % total) + total) % total];
